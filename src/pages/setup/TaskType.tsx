...
import { Setup, taskTypeServices } from "@/services/setupService";
...
const TaskType = () => {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      if (selectedTask) {
        const res = await taskTypeServices.update(selectedTask.id, formData);
        toast({
          title: "Updated",
          description: res.message,
        });
      } else {
        const res = await taskTypeServices.create(formData as Setup);
        toast({
          title: "Created",
          description: res.message,
        })
  ...
  const handleDelete = async () => {
    try {
      const res = await taskTypeServices.delete(selectedTask.id);
      if (res?.message) {
        fetchTaskTypes();
        toast({
          title: "Deleted",
    ...
    <Helpdesk>
      <div className="min-h-screen bg-white p-4">
        ...
        <div className="bg-white flex justify-between items-center">
          <Button
            className="mt-[10px] mb-[6px] h-auto bg-white text-[#333333] text-xs font-normal rounded-[4px] border border-[#ccd3de] px-[10px] py-[2px] hover:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
            onClick={() => {
              setSelectedTask(null);
              reset({ name: "", description: "", color: "#000000" });
              setIsOpen(true);
          ...
          <Table className="min-w-[600px] w-full bg-white text-sm">
            <TableHeader>
              <TableRow className="py-1 border-t">
                ...
                <TableHead className="w-[450px] lg:text-xs font-semibold leading-[36px]">
              ...
              {loading ? (
                ...
                data.map((tasktype) => (
                  <TableRow key={tasktype.id}>
                    ...
                    <TableCell className="py-1.5 text-xs">{tasktype.description}</TableCell>
          ...
          <DialogContent className="sm:max-w-[600px] bg-white px-[16px] py-4">
            <DialogHeader>
              <DialogTitle className="text-lg font-normal">
                {selectedTask ? "Edit Task Type" : "New Task Type"}
              </DialogTitle>
            </DialogHeader>
            <span className="w-full h-[1px] bg-[#f4f4f4] mt-1.5"></span>
            ...
            <form className="space-y-4 px-2" onSubmit={handleSubmit(onSubmit)}>
              <div className="lg:flex sm:flex-none items-center justify-between">
                <div className="flex items-center mb-2 lg:w-[500px] sm:w-none">
                ...
                <Input
                  ...
                  className={`border focus:outline-none focus:ring-2 ${errors.name
              ...
              <div className="lg:flex sm:flex-none items-center">
                <div className="flex items-center mb-2 lg:w-[260px] sm:w-none">
                  <Label htmlFor="color" className="text-sm font-normal">Color</Label>
                </div>
                <Input
                  type="color"
                  id="color"
                  {...register("color")}
                  className="w-[120px] h-[30px] p-1 cursor-pointer lg:me-[175px]"
                />
              </div>

              <div className="flex justify-center gap-4 pt-4">
                <Button type="submit" className="bg-[#4588f0] py-0 px-7 rounded-full hover:bg-[#3774d1] text-sm font-normal">
        ...
        <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
          <AlertDialogContent className="rounded-xl py-2 px-0 bg-white">
            ...
            <AlertDialogDescription className="px-3">
              Do you want to delete the selected Task Type?
            </AlertDialogDescription>
