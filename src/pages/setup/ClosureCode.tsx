...
import { ArrowUp, Settings, ArrowDown, Search } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Setup, closureCodeServices } from "@/services/setupService";
...
const ClosureCode = () => {
  ...
  const fetchClosureCodes = async () => {
    setLoading(true);
    try {
      const result = await closureCodeServices.getAll({
        page: currentPage,
        limit: itemsPerPage,
        sort_by: "name",
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      if (selectedTask) {
        const res = await closureCodeServices.update(selectedTask.id, formData);
        toast({
          title: "Success",
          description: res?.message,
        });
      } else {
        const res = await closureCodeServices.create(formData as Setup);
        toast({
          title: "Success",
          description: res?.message,
        });
  ...
  const handleDelete = async () => {
    try {
      const res = await closureCodeServices.delete(selectedTask.id);
      if (res?.message) {
        fetchClosureCodes();
        toast({
          title: "Success",
          ...
          <DialogContent className="sm:max-w-[600px] bg-white px-[16px] py-4">
            <DialogHeader>
              <DialogTitle className="text-lg font-normal">
                {selectedTask ? "Edit Closure Code" : "New Closure Code"}
              </DialogTitle>
            </DialogHeader>
        ...
        <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
          <AlertDialogContent className="rounded-xl py-2 px-0 bg-white">
            ...
            <AlertDialogDescription className="px-3">
              Do you want to delete selected Closure Code?
            </AlertDialogDescription>
